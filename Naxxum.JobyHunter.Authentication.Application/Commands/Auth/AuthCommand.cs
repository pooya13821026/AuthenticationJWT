    ...
    public class AuthCommandHandler : IRequestHandler<AuthCommand, AuthResponseDTO>
    {
        ...
        public async Task<AuthResponseDTO> Handle(AuthCommand request, CancellationToken cancellationToken)
        {
            ...
            var (userId, fullName, userName, email) =
                await _identityService.GetUserDetailsAsync(await _identityService.GetUserIdAsync(request.UserName));

            string token = _tokenGenerator.GenerateJwtoken((userId, userName));
